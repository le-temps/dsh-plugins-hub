...
- **`Stop` 只支持部分功能：** 阻塞会强制另一个模型轮次，但 `stop_hook_active` 始终为 `false`，`last_assistant_message` 始终为 `null`，且 `{"continue": false}` 会通过取消机制（cancellation）停止运行。连续阻塞上限已实现，最多允许 5 次，以防止死循环。
- **通用 payload 与输出字段只支持部分功能：** 每个已映射事件都报告静态配置的 `model` 与 `permission_mode: "default"`，而非当前 Codex 运行时值。`systemMessage` 会被记录并触发警告，但不呈现，`{"continue": false}` 会通过取消机制（cancellation）停止运行。
