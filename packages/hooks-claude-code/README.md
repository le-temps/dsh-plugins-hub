...
- **`Stop` is partial:** blocking forces another model turn, but `stop_hook_active` is always `false`, `last_assistant_message`, `background_tasks`, and `session_crons` are omitted. Consecutive forced continuations by a blocking Stop hook are capped at 5 to prevent infinite loops.
- **Common payload and output fields are partial:** mapped event payloads omit `prompt_id`, `transcript_path`, `permission_mode`, and `effort` where Claude Code would provide them. `systemMessage` is logged + warned but not surfaced; `{"continue": false}` halts the run via cancellation; `suppressOutput`, `stopReason`, and `terminalSequence` are not applied.
