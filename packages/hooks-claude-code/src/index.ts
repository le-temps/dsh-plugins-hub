...
/** The maximum number of consecutive forced continuations allowed by a Stop hook. */
const MAX_CONSECUTIVE_STOPS = 5
...
export function apply(ctx: Context, config: Config): void {
  ...
  const consecutiveStops = new WeakMap<Agent, number>()
  ...
  async function runPoint(
    ...
    if (merged.stop && opts.agent) {
      opts.agent.cancel({ kind: 'hook', reason: merged.stopReason ?? 'halted by hook (continue:false)' })
    }
    return merged
  }
  ...
  ctx.on('agent/turn-stopping', async ({ agent, turn, signal }): Promise<void> => {
    ...
    if (merged.decision === 'deny') {
      const count = (consecutiveStops.get(agent) ?? 0) + 1
      if (count > MAX_CONSECUTIVE_STOPS) {
        ctx.logger.warn(`hooks-claude-code: Stop hook forced continuation ${count} times; capping to prevent infinite loop.`)
        consecutiveStops.delete(agent)
        return
      }
      consecutiveStops.set(agent, count)
